...
public class LightScan extends Applet implements Runnable {
    ...
    private int myTextSize = 40;// 文本的大小
    private int myTextWidth = 10;// 文本的宽度
    private int fontHeight, baseLine, w;
    public void init()// 小程充的初始化
    {
        myText = "我是世界第一！";// 在小程序中需要显示的文本对像
        fontSize = 30;// 设置字体的大小
        font = new Font("华文琥珀", Font.BOLD, fontSize); // 创建Font对像
        ...
        w = (getSize().width - w) / 2;
        spotPosition = w;
        setBackground(Color.white); // 设置小程序的背景色为白色
    }
    public void start() {
        if (mythread == null) {
